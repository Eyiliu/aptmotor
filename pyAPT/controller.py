...
class Controller(object):
  ...
  def move_raw(self, steps=1, channel=1, wait=True):
    """
    Tells the stage to move some steps.
    ...
    abs_pos_apt_cur = self.position(raw=True)
    abs_pos_apt = abs_pos_apt_cur + steps
    abs_pos_mm = abs_pos_apt / self.position_scale
  ...
  def goto_raw(self, steps=1, channel=1, wait=True):
    """
    Tells the stage to move some steps.
